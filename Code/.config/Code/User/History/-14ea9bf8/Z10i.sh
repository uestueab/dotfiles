...
function f_coom() {
    ...
    then                
        ...
        binding
=$(grep "$model" "$bindingFile" | cut -f2 -d:)
        ...
        if grep -q "$model" "$bindingFile"; then                            #Check whether provided model exists in bindingFile
            binding
    =$(grep "$model" "$bindingFile" | cut -f2 -d:)    #yes?: =>read the service from bindingFile
        else
            binding
    =$(f_getSource)                               #no?: =>bind the service to the model and persist in file
            echo "$model:$binding
    "  >> "$bindingFile" 
        fi
